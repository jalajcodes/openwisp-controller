...
class AbstractConfig(BaseConfig):
    ...
    def get_subnet_division_context(self):
        ...
        for entry in qs:
            if entry['subnet__subnet'] == entry['ip__ip_address']:
                context[entry['keyword']] = entry['rule__size']
            elif entry['ip__ip_address'] is None:
                context[entry['keyword']] = str(entry['subnet__subnet'])
            else:
                context[entry['keyword']] = str(entry['ip__ip_address'])

        # TODO: Find a way to avoid this query
        prefixlen = (
        ...
        if prefixlen:
            context[f'{prefixlen["rule__label"]}_prefixlen'] = prefixlen['rule__size']
        return context
